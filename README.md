...
```bash
geography-game$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
geography-game$ sam local start-api
geography-game$ curl http://localhost:3000/
```
...
```bash
geography-game$ sam logs -n HelloWorldFunction --stack-name geography-game --tail
```
...
```bash
geography-game$ cd HelloWorldFunction
...
```bash
sam delete --stack-name geography-game
```
